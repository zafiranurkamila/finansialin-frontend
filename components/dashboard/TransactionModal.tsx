...
type OcrPayload = {
  ...
  data?: {
    ...
    date?: string | null;
    suggested_category?: number | string | null;
  };
};
...
export function TransactionModal({
  ...
  const normalizeOcrDate = (raw?: string | null) => {
    if (!raw) {
      return "";
    }
  ...
  const handleFileUpload = async (
    ...
    try {
      const result = await apiRequest<OcrPayload>("/ai/receipt-ocr", {
        method: "POST",
        body: formData,
      });

      const ocrData = result?.data ?? (result as any);
      const ocrAmount = ocrData?.total_amount ?? ocrData?.amount ?? "";
      const ocrDate = normalizeOcrDate(ocrData?.date) || initialValues.date;
      const ocrDesc =
        ocrData?.merchant_name ?? ocrData?.description ?? "Hasil Scan OCR";
      const ocrCategory = ocrData?.suggested_category;
      const parsedCategory =
        ocrCategory !== undefined && ocrCategory !== null && ocrCategory !== ""
          ? Number(ocrCategory)
          : undefined;
      ...
      setValues((prev) => ({
        ...
        date: ocrDate,
        type: "expense",
        idCategory: parsedCategory ?? prev.idCategory,
      }));
  ...
  const handleModalSubmit = async () => {
    if (values.idCategory === "new") {
      ...
      try {
        ...
        alert(
          err instanceof Error ? err.message : "Gagal membuat kategori baru.",
        );
      } finally {
      ...
      >
        <div className="modal-head">
          <div>
            ...
            <p>
              {mode === "edit"
                ? "Sesuaikan data transaksi Anda yang sudah ada."
                : "Gunakan ini untuk top up e-wallet, mbanking, cash, atau belanja keluar."}
            </p>
          </div>
          <button
            type="button"
        ...
        {entryMode === "select" && (
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: "0.8rem", color: "#666" }}>
                Ketik sendiri data transaksinya
              </span>
            </button>
          </div>
        )}
        ...
        {entryMode === "ocr" && (
          ...
          >
            ...
            {ocrLoading ? (
              ...
              >
                ...
                <div>
                  ...
                  >
                    Mendukung JPG, PNG, WEBP (Max 4MB)
                  </p>
                </div>
                <span
            ...
            {ocrError && (
              <p className="modal-error" style={{ marginTop: "16px" }}>
                {ocrError}
              </p>
            )}
          </div>
            ...
            <label className="field modal-field">
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setValues((current) => ({
                    ...current,
                    idCategory: val === "new" ? "new" : Number(val),
                  }));
                }}
              >
